...
func WriteToSheet(sheetID string, sheetName string, URLObjectList URLObjectList) error {
...
func WriteWild(URLObjectList URLObjectList) {
	...
	if err := WriteToSheet(secrets.SheetID, secrets.SheetName, URLObjectList); err != nil {
		fmt.Printf("[!] Error writing to sheet: %v", err)
	}
}
